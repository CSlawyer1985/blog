...
assert.equal((html.match(/class="project-item /g) || []).length, 7);
assert.equal((html.match(/class="channel"/g) || []).length, 6);
assert.equal((about.match(/class="proj-item"/g) || []).length, 9);
for (const url of ['https://dsh.chenshi.ai/', 'https://memoball.chenshi.ai/', 'https://skill.chenshi.ai/', 'https://learn-agent.legalagi.cn/']) {
...
console.log('PASS: homepage/template, identity, 7 projects, 6 channels, 9 about entries, preserved repos, local assets, anchors, SEO');
